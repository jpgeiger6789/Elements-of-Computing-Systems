...
class Parser():
    ...
    def compileTerm(self, parent, variableSet):
        ...
        #next handle the terms that are more complicated
            ...
            if not self.nextToken[1][1] in ["[", "(", "."]:
                ...
                if self.nextToken[1][1] != "]":
                    raise ParseException("compileTerm error: an indexed variable must end with a closing bracket", self.nextToken, self.tokenList, self.rootXMLElement, self.tokenizer.iname)
                subEl = ET.SubElement(parent, self.nextToken[1][0])
                subEl.text = self.nextToken[1][1]
                self.nextToken = next(self.tokenIterable)
            ...
            if self.nextToken[1][1] != ")":
                raise ParseException("compileTerm error: an open parentheses in a term must be an expression with a closing parenthesis. Expression was parsed but the following " +
                                    "token was not a closing parenthesis.", self.nextToken, self.tokenList, self.rootXMLElement, self.tokenizer.iname)
            subEl = ET.SubElement(parent, self.nextToken[1][0])
            subEl.text = self.nextToken[1][1]
            self.nextToken = next(self.tokenIterable)
    ...
    def compileSubroutineCall(self, parent, variableSet, previousIdentifier = ""):
        ...
        if self.nextToken[1][1] not in ["(", "."]:
            raise ParseException("compileSubroutineCall error: the token after the first identifier in a subroutine call must be an opening parenthesis or a period", self.nextToken, self.tokenList, self.rootXMLElement, self.tokenizer.iname)
        ...
        if self.nextToken[1][1] == ".":
            ...
            self.nextToken = next(self.tokenIterable)
            if self.nextToken[1][0] != "identifier":
                raise ParseException("compileSubroutineCall error: the token after the period in a class-subroutine call must be an identifier indicating a subroutine name", self.nextToken, self.tokenList, self.rootXMLElement, self.tokenizer.iname)
            subEl = ET.SubElement(parent, self.nextToken[1][0])
            subEl.text = self.nextToken[1][1]
            self.nextToken = next(self.tokenIterable)
        else:
        ...
        if self.nextToken[1][1]  != "(":
            raise ParseException("compileSubroutineCall error: the token after the subroutine name must be an opening parenthesis", self.nextToken, self.tokenList, self.rootXMLElement, self.tokenizer.iname)
        ...
        if self.nextToken[1][1] != ")":
            raise ParseException("compileSubroutineCall error: the token after the expression list must be a closing parenthesis", self.nextToken, self.tokenList, self.rootXMLElement, self.tokenizer.iname)
        subEl = ET.SubElement(parent, self.nextToken[1][0])
        subEl.text = self.nextToken[1][1]
        self.nextToken = next(self.tokenIterable)
